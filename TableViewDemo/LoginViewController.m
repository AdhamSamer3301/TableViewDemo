...
- (IBAction)signUpAlert:(id)sender {
    ...
    UIAlertAction *action = [UIAlertAction actionWithTitle:@"Sign Up" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self->_def setObject:[[alert textFields][0] text] forKey:@"UserName"];
        [self->_def setObject:[[alert textFields][1] text] forKey:@"PassWord"];
        [self->_def setBool:YES forKey:@"isSignedUp"];
        [self->_def setBool:YES forKey:@"isLogged"];
    }];
